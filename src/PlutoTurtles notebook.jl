...
# ╔═╡ 09fff0f8-fc53-4435-bfb7-e275b0233de7
begin
	struct SVGDisplay
		data
	end
	Base.show(io::IO, m::MIME"image/svg+xml", s::SVGDisplay) = write(io, s.data)
	SVGDisplay
end
...
function turtle_drawing_fast(f::Function; 
	...
	return SVGDisplay(make_svg(🐢; background, size))
end
...
function turtle_drawing(f::Function; 
	...
	children = [
		SVGDisplay(make_svg(🐢; background, size)),
		@htl("""
		<script>
		const _x = $(rand())
	...
	AbstractPlutoDingetjes.Display.ReactDOMElement(
		...
		attributes=Dict(
			...
			:style => "position: relative; contain: content; overflow: hidden;",
		),
	)
end
...
manifest_format = "2.0"
project_hash = "a6dfa01ae80b4a36d30e2204b89aaa9e83b7ad8c"
